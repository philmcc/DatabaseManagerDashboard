...
export function registerRoutes(app: Express): Server {
  ...
  // Update metrics endpoint with proper data parsing
  app.get("/api/databases/:id/metrics", requireAuth, async (req, res) => {
    try {
      ...
      if (!database || !database.instance) {
        return res.status(404).json({
          message: "Database or instance not found"
        });
      }

      // Create connection to get metrics
      const client = new Client({
      ...
      try {
        ...
        const activeConnections = parseInt(connectionsResult.rows[0].count) || 0; // Ensure we have a number, default to 0
        ...
        // Parse and validate metrics data
        const metricsData = {
          databaseSize,
        ...
        // Store metrics in database with validated data
        const [storedMetrics] = await db.insert(databaseMetrics).values({
          databaseId,
          activeConnections,  // Number from parseInt
          databaseSize,      // String from pg_size_pretty
          tableStats: tableStatsResult.rows,
          metrics: metricsData,
          timestamp: new Date()
        }).returning();
